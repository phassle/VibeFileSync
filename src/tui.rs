...
pub fn run(config_path: &Path, requested_pair: Option<&str>) -> Result<i32, AppError> {
    ...
    let pair_name = match requested_pair {
        ...
        // Startup's empty-config and lone-pair auto-select behaviours are
        // sibling ticket #55's territory (untouched here); this ticket only
        // adds create/edit/remove once the selector is already reachable.
        ...
        None => match select_pair(config_path)? {
            Some(name) => name,
...
/// Opens the pair selector and, from it, the create/edit/remove flows this
/// ticket adds. The selector owns a reload loop so that a save or removal —
/// which goes through `pair::add`/`pair::remove`, never a config write here —
/// is reflected immediately without tearing the screen down.
fn select_pair(config_path: &Path) -> Result<Option<String>, AppError> {
    let mut session = TerminalSession::start().map_err(tui_error)?;
    let mut events = CrosstermEvents::default();
    let mut choices = pair_choices(&config::load(config_path)?);
...
fn select_pair_loop<B: Backend, E: Events>(
    ...
    loop {
        ...
        terminal
            .draw(|frame| draw_pair_selector(frame, choices, *selected, header_mode))
            .map_err(tui_error)?;
        let Event::Key(key) = events.next().map_err(tui_error)? else {
            continue;
        };
        ...
        match key.code {
            ...
            KeyCode::Char('n') | KeyCode::Char('N') => {
                ...
                let outcome = pair_form_loop(terminal, events, &mut form, config_path, header_mode)
                    .map_err(tui_error)?;
                reload_after_form(config_path, choices, selected, outcome)?;
            }
            ...
            KeyCode::Char('x') | KeyCode::Char('X') => {
                if let Some(choice) = choices.get(*selected).cloned() {
                    ...
                    {
                        *choices = pair_choices(&config::load(config_path)?);
                    }
                }
            }
...
/// Reloads `choices` from disk after a create/edit form closes, and — if it
/// saved — moves the selector's cursor onto the saved pair so the just-
/// edited entry stays highlighted rather than jumping back to the top.
