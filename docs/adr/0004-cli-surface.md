...
3. **Per-run flags** (all per-run only, never config, per ADR-0001/0002/0003): `--yes`, `--json`, `--permanent-delete`, `--allow-empty-source`, `--ignore-space-check`, and repeatable `--exclude <relative-path>` — exact paths as the plan prints them, no glob engine in v1. This gives scripted runs parity with the TUI's per-row include/exclude.
...
5. **Schema stability rule:** every JSON payload carries a `schema` field; changes within `/v1` are additive-only — removing a field or changing a meaning bumps the version. `pair list --json` and `history --json` follow the same convention (`vibefilesync.pairs/v1`, `vibefilesync.history/v1`).
...
- `--exclude` takes exact plan paths, so the agent workflow is: `plan --json` → filter rows → `run --exclude … --yes`. Glob support, if ever, is additive.
