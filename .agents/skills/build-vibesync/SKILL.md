...
1. Confirm supported host with `uname -srm`. Expect Darwin arm64; platform calls are concentrated at `src/volume.rs:37` and `src/run.rs:31`.
...
Keep `--locked` so dependency drift fails visibly. Update `Cargo.lock` only when dependency/version work is in scope. Never use a real `vibesync run` as a build smoke test; Run mutates its configured destination (`src/run.rs:59`).
